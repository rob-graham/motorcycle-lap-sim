...
The deterministic bounded coordinate search starts at zero unless a validated
warm-start control vector is supplied, tries `+` then `-`, uses local asymmetric
physical bounds, and follows Phase 5 step reduction. It is local, so no global
optimum is claimed. A finer model is not automatically more physically
accurate. Material policy dependence is reported as unconverged path-model
sensitivity rather than tuned away. The model adds no clothoids, centreline
smoothing, banking, elevation, kerbs, calibration changes, or curvature-rate
limit.
...
agree within the configured tight backend-equivalence tolerances before the
Python result becomes the reported profile.
...
Both the generic user-facing `motorcycle-lap-sim optimise` command and the
historical Phase 8 diagnostic can warm-start from an earlier strict physical
controls CSV.

For the generic command:

```text
motorcycle-lap-sim optimise TRACK.yaml MOTORCYCLE.yaml --policy reference --restart-controls previous_controls.csv --initial-step-m 0.25 --output continued_controls.csv
```

`--initial-controls-csv` is an alias for `--restart-controls` on the generic
command. The historical diagnostic retains its existing
`--initial-controls-csv` spelling.

This is a strict same-track-layout, same-policy and same-boundary-margin
restart. The saved station layout and bounds must match those generated by the
current track, selected policy and boundary margin. Controls are never
reordered, interpolated, projected, or clipped. A different motorcycle YAML is
permitted because the controls file supplies geometry only; the new motorcycle
configuration is used to evaluate and optimise the entire restarted run.

Controls and the search step are separate pieces of restart state: the CSV
restores only the physical control vector, while `--initial-step-m` explicitly
sets the new run's starting search step and defaults to `1.0 m` when omitted.
Evaluation count, sweep count, and process-pool state begin fresh from the
current `PlanarOptimisationConfig`. The restart CSV format is unchanged.

The Phase 8 diagnostic equivalent is, for example:
...
Long targeted diagnostic runs may pass `--checkpoint-controls-csv PATH`. After
every complete poll (and its optional pattern move), the parent process
atomically replaces this file in the strict controls format accepted by the
diagnostic's `--initial-controls-csv`. For example:
