...
and pushes the named task branch before starting the Implementer:

```bash
TASK_BRANCH="task/<short-description>"
git switch main
git pull --ff-only
git status --short
...
Do not proceed unless `git status --short` is empty. Send the Local Implementer the repository,
...
CLI is available, use this procedure with the full reviewed SHA:

```bash
PR_NUMBER="<PR-number>"
REVIEWED_HEAD_SHA="<full-reviewed-head-sha>"
...
Stop if either `git status --short` is non-empty, either SHA comparison fails, or any required
...
```bash
LOCAL_PR_BRANCH="<local-branch-created-by-checkout>"
git switch main
git pull --ff-only
